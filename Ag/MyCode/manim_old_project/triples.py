...
class IntroduceTriples(TeacherStudentsScene):
    def construct(self):
        ...
        for a, b, c in triples:
            ...
            # 定义空VMobject()对象
            elbow = VMobject()
            # 使用set_points_as_corners()增加self.points
            ...
            # c_square_parts = a_square + b_square
...
class ShowManyTriples(Scene):
    def construct(self):
        # 生成所有的直角三角形 前40个 如[(3, 4, 5),(5, 12, 13), ... ]
        triples = [
            (u**2 - v**2, 2*u*v, u**2 + v**2)
            for u in range(1, 15)
            for v in range(1, u)
            if fractions.math.gcd(u, v) == 1 and not (u%2 == v%2)
        ][:40]
        triangles = VGroup()
        titles = VGroup()
...
class AskAboutFavoriteProof(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_thinks("这里不能为空", target_mode = "thinking")
        self.wait()
