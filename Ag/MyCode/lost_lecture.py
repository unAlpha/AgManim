...
class Orbiting(Mobject):
    ...
    def __init__(self, planet, star, ellipse, **kwargs):
        ...
        planet.add_updater(self.update_mobject)
...
class SunAnimation(Mobject):
    ...
    def __init__(self, sun, **kwargs):
        ...
        Group(sun, self.rotated_sun).add_updater(self.update_mobject)
...
class ShowEmergingEllipse(Scene):
    ...
    def construct(self):
        ...
        self.add(ghost_line)
        self.play(
            MoveToTarget(line, path_arc=90 * DEGREES),  # 生成新和目标 给定属性 并有动画
            Animation(rot_words),
            ShowCreation(elbow)
        )
        self.wait()
    ...
    def get_lines(self):
        ...
        for line in lines:
            ...
            line.target.rotate(90 * DEGREES)  #要有目标
        return lines
...
# 加入Feynman的图
    def construct(self):
        ...
        self.add(group)
        self.add(Orbiting(comet, e_dot, ellipse,rate=2.0))
        self.add_foreground_mobjects(group)  # 添加前景对象
        self.wait()
        self.play(
            feynman.restore,
...
# 行星围绕太阳转
    ...
    def get_planets_ellipses_and_orbits(self, sun):
        ...
        orbit_eccentricies = [0.206, 0.006, 0.0167, 0.0934, 0.967]  # 轨道半径偏心
