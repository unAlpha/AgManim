...
class TakeOver(PiCreatureScene):
    ...
    def construct(self):
        gradient = ImageMobject("Lost_lecture/white_black_gradient")
        # white black gradient 白色黑色渐变
        gradient.set_height(FRAME_HEIGHT)
        self.add(gradient)

        morty = self.pi_creatures
        henry = ImageMobject("Lost_lecture/Henry_As_Stick")
        henry.set_height(4)
        henry.to_edge(LEFT)
        henry.to_edge(DOWN)
...
class ShowEmergingEllipse(Scene):
    ...
    def construct(self):
        ...
        line = lines[len(lines) // 5]  # 向下取整  取得取中一根线
        ...
        elbow = self.get_elbow(line)  # 直角标示
        ...
        self.play(
            FadeInFrom(e_dot, LEFT),  # 淡入然后再移动到
            Write(eccentric_words, run_time=1)
        )
        self.wait()

        self.play(
            LaggedStartMap(ShowCreation, shuffled_lines),  #对VGroup 生成动画
        ...
        self.play(
            FadeIn(fade_rect),  # 变暗0.7整个屏幕
            Animation(line),
            GrowFromCenter(line_dot),  # 从中间变大呈现
            FadeInFromDown(rot_words),  # 从下方淡入进去
        )
        self.wait()
        self.add(ghost_line)
        self.play(
            # 生成新和目标 给定属性 并有动画
            # path_arc=90 * DEGREES 这里无效，本身动画也是这个效果
    ...
    def get_lines(self):
        ...
        radius = self.circle.get_width() / 2   # 半径
        e_point = self.get_eccentricity_point()  # 偏心点
        lines = VGroup(*[
            Line(
                e_point,
...
class ShowFullStory(Scene):  
    def construct(self):
        directory = os.path.join(
            MEDIA_DIR,
            "assets/raster_images/Lost_lecture/images"
        )
        scene_names = [
            "ShowEmergingEllipse",
            "ShowFullStory",
...
class FeynmanAndOrbitingPlannetOnEllipseDiagram(ShowEmergingEllipse):
    def construct(self):
        ...
        comet = ImageMobject("Lost_lecture/earth")
        comet.set_width(0.3)
        
        feynman = ImageMobject("Lost_lecture/Feynman")
        ...
        self.add(circle, ghost_lines, lines, e_dot, )
        self.play(LaggedStartMap(MoveToTarget, lines, run_time=4))
        self.play(FadeIn(ellipse))
        self.add(Orbiting(comet, e_dot, ellipse, rate=2))
        # 这个很重要相当于add
...
class FeynmanFame(Scene):
    ...
    def construct(self):
        books = Group(
            ...
            ImageMobject("Lost_lecture/Surely_Youre_Joking_cover"),
            ImageMobject("Lost_lecture/Feynman_Lectures_cover"),
        )
        for book in books:
            book.set_height(6)
        ...
        feynman_smile = ImageMobject("Lost_lecture/Feynman_Los_Alamos")
        feynman_smile.set_height(4)
        feynman_smile.next_to(objects, DOWN)

        Group(objects, feynman_smile).next_to(ORIGIN, RIGHT)
    ...
    def get_book_animation(self, book,
        ...
        rect = BackgroundRectangle(book, fill_opacity=1, buff=0.1)
...
class TheMotionOfPlanets(Scene):
    ...
    def setup_orbits(self):
        sun = ImageMobject("Lost_lecture/sun")
        sun.set_height(0.7)
        ...
        book = ImageMobject("Lost_lecture/Lost_Lecture_cover")
        book.set_height(4)
        book.next_to(alt_name, DOWN)

        sun_anim = SunAnimation(sun)
        self.add(sun_anim)
        ...
        self.wait(3)
        self.play(
            Group(ellipses,*sun_anim).shift, 3 * RIGHT,
    ...
    def get_planets_ellipses_and_orbits(self, sun):
        planets = Group(
            ImageMobject("Lost_lecture/mercury"),
            ImageMobject("Lost_lecture/venus"),
            ImageMobject("Lost_lecture/earth"),
            ImageMobject("Lost_lecture/mars"),
            ImageMobject("Lost_lecture/comet")
        )
        sizes = [0.383, 0.95, 1.0, 0.532, 0.3]
        orbit_radii = [0.254, 0.475, 0.656, 1.0, 3.0]  # 轨道半径
        orbit_eccentricies = [0.206, 0.006, 0.0167, 0.0934, 0.967]  # 轨道偏心率
...
class ShowEllipseDefiningProperty(Scene):
    ...
    def construct(self):
        self.add_ellipse()  # 屏幕上显示Ellipse
        self.add_focal_lines()  # 添加焦线
        self.add_distance_labels()  # 显示数字
        self.label_foci()
    ...
    def add_ellipse(self):
        ...
        ellipse.stretch(fdiv(b, a), dim=1)  # stretch 拉伸
    ...
    # Helpers
    def get_foci(self):  # 获得焦点 返回一个list
        ellipse = self.ellipse
...
# 孤形字样 效果爆炸
    ...
    def get_geometry_proof_land_word(self):  # 返回一个最终效果
        ...
        word.scale(0.25)
        word.shift(3 * RIGHT)
        word.apply_complex_function(np.exp)  # 应用复杂的功能
        word.rotate(90 * DEGREES)
        word.set_width(9)
        word.center()
        word.to_edge(UP)
