...
class RotationObject(Scene):
    def construct(self):
        textM = TextMobject("Text")
        textC = TextMobject("Reference text")
        textM.shift(UP)
        textM.rotate(PI/4) #El centro es el centro de area del texto, en radianes, texto u objeto
        self.play(Write(textM),Write(textC))
        self.wait(2)
        textM.rotate(PI/4)
        self.wait(2)
...
class FontsText(Scene):
	def construct(self):
		textNormal = TextMobject("{Roman serif text 012.\\#!?} Text")
		textItalica = TextMobject("\\textit{Italic text 012.\\#!?} Text")
		textMaquina = TextMobject("\\texttt{Typewritter text 012.\\#!?} Text")
		textNegritas = TextMobject("\\textbf{Bold text 012.\\#!?} Text")
		textSL = TextMobject("\\textsl{Slanted text 012.\\#!?} Text")
		textSC = TextMobject("\\textsc{Small caps text 012.\\#!?} Text")
		textNormal.to_edge(UP)
		textItalica.next_to(textNormal,DOWN,buff=.5)
		textMaquina.next_to(textItalica,DOWN,buff=.5)
		textNegritas.next_to(textMaquina,DOWN,buff=.5)
		textSL.next_to(textNegritas,DOWN,buff=.5)
		textSC.next_to(textSL,DOWN,buff=.5)
		self.add(textNormal,textItalica,textMaquina,textNegritas,textSL,textSC)
		self.wait(3)
