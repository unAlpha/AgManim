...
class ShowSimpleMultivariableFunction(Scene):
    def construct(self):
        ...
        many_inputs.set_width(FRAME_WIDTH-1)
        ...
        # )
        decimal.add_updater(
            lambda a: a.set_value(1 + np.sin(value_tracker.get_value()))
            )
        self.add(decimal)
...
class ShowFunction(Scene):
    def construct(self):
        ...
        func.scale(1.5)
        self.play(FadeInFromLarge(func))
        self.wait()
...
class ShowDerivativeAtExamplePoint(Scene):
    ...
    def get_result_vector(self, x, y):
        ...
        x_terms = VGroup(
            result_vector.get_mob_matrix()[0, 0],
            result_vector.get_mob_matrix()[1, 0],
            result_vector.get_mob_matrix()[0, 0],
        )
        y_terms = VGroup(
            result_vector.get_mob_matrix()[0, 0],
